...
def _get_within_transcript_effects(annotator, base_effect, transcript,
    ...
    if not cdss:
        ...
        overlapping_cdss = [cds for cds in cdss
                            if cds.start <= ref_stop and cds.stop >= ref_start]

        overlapping_introns = [(start, stop) for (start, stop) in introns
...
def _get_within_intron_effect(annotator, base_effect, intron, cdss):
    ...
    if strand == '+':
        ...
        intron_cds_5prime = [cds.feature_id for cds in cdss if cds.stop == intron_start - 1][0]
        intron_cds_3prime = [cds.feature_id for cds in cdss if cds.start == intron_stop + 1][0]
    else:
        intron_5prime_dist = (intron_stop + 1) - ref_stop
        intron_3prime_dist = (intron_start - 1) - ref_start
        intron_cds_3prime = [cds.feature_id for cds in cdss if cds.stop == intron_start - 1][0]
        intron_cds_5prime = [cds.feature_id for cds in cdss if cds.start == intron_stop + 1][0]
