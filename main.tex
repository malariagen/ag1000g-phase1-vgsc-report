...
\title{
The genomics of target-site resistance to pyrethroid insecticides in the African malaria vectors \emph{Anopheles gambiae} and \emph{Anopheles coluzzii}
}
...
%
Using networks and hierarchical clustering methods we identify ten distinct haplotype clusters caused by selective sweeps at this resistance locus, of which five appear to be localised to a single geographical location, and five have spread between two or more countries.
%
The most successful and widespread haplotype cluster (F1) originates in West Africa and has subsequently spread to countries in Central and Southern Africa.
%
Our results demonstrate that the molecular basis of pyrethroid resistance in African malaria vectors is much more complex than previously appreciated, and provide a foundation for the design of new genetic tools to track the spread insecticide resistance and to inform vector control.
%%
...
%
To our knowledge, no studies in malaria vectors (prior to \cite{Ag1000gConsortium2017}) have analysed the full \emph{Vgsc} coding sequence, thus the genetic basis of target-site resistance to pyrethroids has not been fully explored.
%%
...
%
Previous studies have found evidence that \texttt{L995F} occurs on several different genetic backgrounds, suggesting multiple independent ``origins'' of resistance driven by this allele \cite{Pinto2007,Etang2009,Santolamazza2015}. 
%
However, these studies analysed only a small region of the VGSC gene, and therefore had limited power to make inferences about the selective events or spread of resistance alleles. 
%
...
%
We investigate variation across the complete gene coding sequence, to fully characterise the variants potentially driving primary and secondary selective sweeps of target-site resistance to pyrethroids in natural mosquito populations.
%
...
%
The goal of these analyses is to diagnose how many separate selective events have occurred, which are localised, and which are spreading.
%
Finally we explore ways in which variation data from Ag1000G could be used to design high-throughput, low-cost genetic assays for monitoring pyrethroid resistance, with the capability to differentiate and track separate resistance driven selective sweeps.  
%%
...
%
To identify variants with potentially functional roles in pyrethroid resistance, we extracted single nucleotide polymorphisms (SNPs) from the Ag1000G phase 1 data resource that alter the amino acid sequence of the VGSC protein, and computed their allele frequencies among 9 populations defined by species and country of origin.
%
Alleles that confer resistance are expected to increase in frequency under selective pressure, and we refined the list of potentially functional variant alleles to retain only those at an appreciable frequency ($\geq$5\%) in one or more populations (Table \ref{table:variants_missense}).
%
...
%
We reported 15 of these novel alleles in our initial analysis of the Ag1000G phase 1 data \cite{Ag1000gConsortium2017}, and we extend the analyses here to incorporate a SNP which alters codon 1603 and the tri-allelic SNPs at codons 402 and 490.
%%
...
All variants are at 5\% frequency or above in one or more of the 9 Ag1000G phase 1 populations, with the exception of \texttt{2,400,071 G>T} which is only found in the CM\emph{Ag} population at 0.4\% frequency but is included because another mutation (\texttt{2,400,071 G>A}) is found at the same position causing the same amino acid substitution (\texttt{M490I}); and \texttt{2,431,019 T>C} (\texttt{F1920S}) which is at 4\% frequency in GA\emph{Ag} but also found in CM\emph{Ag} and linked to \texttt{L995F}. 
  ...
  (Table \ref{table:variants_missense})
  \begin{tablenotes}
    \footnotesize
...
%
289 samples in the dataset carried $\geq$1 \texttt{L995F} alleles of which 92 were homozygotes and in the case of \texttt{L995S}, 156 samples carried $\geq$1 alternative alleles and of these 38 were homozygotes;
%
the resistance phenotype at this locus is thought to be incompletely recessive \cite{Chandre2000}.
%
Both alleles were present in populations sampled from Cameroon and Gabon, including some individuals with a hybrid \texttt{L995F/S} genotype (46/275 samples in Cameroon, 36/56 in Gabon).
%
In Cameroon these alleles were in Hardy Weinberg equilibrium ($x^{2}$ = 0.02, p > 0.05), thus there does not appear to be selection for, or against, heterozygote carriers of both alleles; however in Gabon, they were not in equilibrium ($x^{2}$ = 8.96, p < 0.005), with an excess of heterozygotes suggesting there may be a fitness advantage to mosquitoes carrying both alleles in this region.
%%
...
%
These include the \texttt{N1570Y} allele, known to enhance pyrethroid resistance in \textit{An. gambiae} in combination with \texttt{L995F} \cite{Jones2012} and also two variants in codon 1874 (\texttt{P1874S}, \texttt{P1874L}). 
%
\texttt{P1874S} has previously been found in a colony of the crop pest \textit{Plutella xylostella} with a pyrethroid resistance phenotype, but has not been shown to confer resistance experimentally \cite{Sonoda2008}.
%
...
%
All 3 variants occur in internal linker domains, and so do not fit the model of a resistance driver, again experimental work is required to rule out a resistance phenotype.
%
%%
...
% Paragraph 6 - introduce haplotypes
...
%% Paragraph 7 - breifly acknowledge previous work
%
...
%% Paragraph 8 - Introduction to the networks.
%
...
%
This analysis is  similar to hierarchical clustering, however, it allows for the reconstruction and placement of intermediate haplotypes that may not be observed in the data and allows the relationships of the closely related clustered haplotypes to be discerned.
%
...
%% Paragraph 9 - Observations from the networks.
%
...
%
While the genetic-distance based hierarchical clustering and network analyses provide evidence for the spread of resistance alleles via adaptive gene flow, and for the secondary evolution of \texttt{L995F} enhancer alleles, they have several limitations.
%
...
%
Finally, analyses of genetic distance within a fixed genome region can be confounded by recombination events.
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection*{Shared haplotype length}

%%
To overcome the problems of fix windowed analysis, we estimated the length of the region shared identical by descent (IBD) (@@?? and the number of mutations that have accumulated since the most recent common ancestor - methods).
%
By measuring shared haplotype length from genome regions both within and flanking the \textit{Vgsc}, the relationship between haplotypes could be examined in a higher resolution.
%
Visualisation of the pairwise IBD length distribution revealed bi-modality; a minor mode of haplotypes with longer shared haplotypes and a major mode of haplotypes with shorter tracts of IBD (Figure \ref{fig:age_hist}).
%
It is well established that the \textit{Vgsc} locus is under insecticide mediated recent selection and that there have been multiple selective events e.g. \cite{Ag1000gConsortium2017} (@@cite more) and therefore this bi-modal pattern is expected.
%
Within each selective event, pairs of haplotypes will share a more recent common ancestor on average and therefore will have longer tracts of IBD - generating the minor mode.
%
In the absence of recent selection and in the presence of large effective population sizes \cite{Ag1000gConsortium2017} (@@cite more), pairs of haplotypes will have less recent common ancestors on average, with tracts of IBD broken up by recombination over time - the major mode.
%
The bimodal distribution is not likely due to geographical population structure, because the same pattern is observed within several populations.
%%

%
Pairwise IBD lengths were then hierarchically clustered to build a dendrogram.
%
The midpoint between the two modes of IBD distribution was taken as an estimation of the minimum length of IBD tract associated with a recent selective event and this was used to cut the IBD dendrogram, defining  haplotype clusters representing putative selective events (Figure \ref{fig:tree}).
%
Comparing this to previous analyses of haplotype structure based on genetic distance, we find clusters F1-F5 and S1-S3 recapitulated with close correspondence (@@ how close), and S4 and S5 merged into a single cluster.
%
We label a new cluster ``L1'' representing an outbreak driven by the \texttt{I1527T} allele in combination with one or the other \texttt{V402L} allele.
%
We also label a cluster ``L2'' capturing a set of haplotypes from Kenya carrying the \texttt{M490I} variant, although the fact that these haplotypes all share a recent common ancestor may be a reflection of the unusual demography of the Kenyan population which has experienced a severe population crash and not be due to recent selection for insecticide resistance \cite{Ag1000gConsortium2017}.
%
As in earlier analyses (Figure \ref{fig:networks}; \cite{Ag1000gConsortium2017}), clusters F1, F4, F5, S2 and S3 all include haplotypes sampled from multiple geographical locations, and F1 includes haplotypes from both species, \textit{An. gambiae} and \textit{An coluzzii}.
%
Clusters F2, F3, S1, S4/5, L1 and L2 include only haplotypes from a single sampling location.
%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection*{Haplotype selection}

%%
Good concordance was found across clustering analyses (@@make supp table) with higher resolution of haplotype structure in the IBD analyses through extending up- and down-stream of the \textit{Vgsc} locus.
%
To confirm these IBD generated clusters are indeed experiencing recent selection and to provide an alternative view of how haplotypes are related, analysis of extended haplotype homozygosity (EHH) decay was performed \cite{Sabeti2002}.
%
We defined a core region spanning \textit{Vgsc} codon 995 and an additional 4 kbp of flanking sequence (Methods).
%
Within this core region, we found 18 distinct haplotypes at a frequency > 1\% within the cohort, including core haplotypes representing each of the resistance outbreaks we identified above, and a further 7 core haplotypes not carrying any known or putative resistance allele for comparison (wild-type - wt).
%
Comparision of the core haplotypes from clusters defined by IBD with the wild-type core haplotypes reveal clear reductions in the EHH decay up- and down-stream of all but one core haplotypes, with only L2 from Kenya not showing reduced EHH decay on the left flank (Figure \ref{fig:ehh_decay})
%
EHH decay generally decays more slowly in the centromere proximal direction.
% 
These results suggest that haplotypes carry non-synonymous mutations in the \textit{Vgsc} gene are under recent positive selection unlike the wild-type haplotypes which do not carry protein altering mutations.
%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection*{Haplotype recombination}

%%
To study the evidence for recombination within region spanning the \textit{Vgsc} gene, and provide some additional confirmation that our inferences regarding insecticide resistance haplotype clusters have not been affected by recombination or other sources of error, we performed an additional analysis of genetic distance between haplotypes.
%
...
%
We then computed the genetic distance ($D_{XY}$) between each of our sampled haplotypes and each of the inferred ancestral outbreak haplotypes, computing the distance in 2000bp windows (step size 1000bp) across a 5 Mbp region spanning the \textit{Vgsc} gene.
%
The results for clusters F1-F5 are plotted in Figure \ref{fig:recom_f}, S1-S4/5 are shown in Figure \ref{fig:recom_s} and L1-L2 in Figure \ref{fig:recom_l}.
%%
...
%%
In these plots we expect that all haplotypes from a given cluster should share very close genetic similarity ($D_{XY} \approx 0$) with each other and with the ancestral haplotype for that cluster within the \textit{Vgsc} gene itself, with an increasing number of haplotypes recombining away from the ancestral haplotype as we move away from the gene in either the upstream or downstream direction.
%
Conversely, haplotypes from one cluster should not share any close genetic similarity ($D_{XY} > 0$) with the inferred ancestral haplotype from a different cluster, either within the \textit{Vgsc} gene or in flanking regions.
%
The results for all outbreaks are largely consistent with this expectation.
%
For this analysis we treated S4/5 as a single cluster, as indicated by the IBD analysis, and we can gain some insight into why these two were split into separate clusters in earlier analyses.
%
All haplotypes in S4/5 share close similarity with the ancestral haplotype on both flanks of the \textit{Vgsc} gene, but there is a short region of within the gene where a subset of haplotypes are diverged.
%
This region of divergence accounts for the S4/S5 split in earlier analyses (Figure \ref{fig:networks}).
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection*{Design of genetic assays for haplotype surveillance}
...
%%
%
To facilitate the development of targeted genetic assays for \textit{Vgsc} insecticide resistance haplotype surveillance, we have produced two supplementary data tables.
%
...
%
Not all SNPs are informative for detecting whether an individual mosquito carries a haplotype from a haplotype cluster under selction, and we provide some summary statistics for each SNP to aid in the identification of the most informative SNPs.
%
This includes allele frequencies within each of the clusters identified here, as well as for populations of susceptible haplotypes.
%
...
%
This analysis suggests that it should be possible to construct a tree able to classify haplotypes from all 10 resistance selective sweeps with >95\% accuracy using 20 SNPs or less.
%%
