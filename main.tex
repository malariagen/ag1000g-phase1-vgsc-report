...
%
This analysis suggests that by using genomic data it should be possible to construct a decision tree able to classify these resistance haplotypes with >95\% accuracy by using 20 SNPs or less - a tractable number for molecular assay design, which would allow detection of haplotypes in the field.
%%
...
%%
%
Both indirect (clustering, networks) and direct evidence (EHH) for selection on \textit{Vgsc} haplotypes we detected, can be used in concert with knowledge of the gene's structure, linkage of variants, and evidence of phenotypes from both \textit{Anopheles} and other insect systems, to predict putative resistance phenotypes for the novel non-synonymous variants detected.
%
These predictions could help define relationships between newly detected variants and prioritise assay design for molecular tracking of variants/haplotypes in the field.
%
To explore this potential for predication, we divide the resistance phenotypes into known (experimentally shown) and putative, and divide the causative variants into phenotype drivers or enhancers.
%
Driver variants occur on haplotypes first, initiating the resistance phenotype and starting the selective sweep. 
%
Experimentally established resistance variants \texttt{L995F}, \texttt{L995S} in \textit{An. gambiae} and \texttt{V402L} in \textit{A. aegypti} (\textit{An. gambiae} codon numbering) are examples of known driver variants (Table \ref{table:variants_pheno}). 
%
Enhancer variants occur on haplotypes already undergoing selective sweeps due to driver variants, these enhancer variants either increase the resistant phenotype above that of the driver variants alone, or ameliorate fitness costs of carrying a driver. 
%
The \texttt{N1570Y} variant, which has only been found on haplotypes carrying \texttt{L995F} and, using \textit{Xenopus} oocytes, has been shown to produce an increased resistance phenotype only when paired with \texttt{L995F} \cite{Wang2015}, is thus classed as an enhancer variant (Table \ref{table:variants_pheno}). 
%
%%


%we have described all the mutations in detail in the first part of the results section. I'm not sure what to add now to justify table 2 inclusion here? 
...
\textbf{Phenotype prediction in the voltage-gated sodium channel gene}. 
  ...
  \begin{tablenotes}
    ...
    \item[4] One residue away from known driver mutation \texttt{F1534C} (\emph{Musca domestica} codon numbering).
...
%
We used variant call data from the phase 1 AR3 release and phased haplotype data from AR3.1. These data are publically downloadable via ftp from \url{https://www.malariagen.net} and are archived in the European Nucleotide Archive (ENA; http://www.ebi.ac.uk/ena) under study PRJEB18691.
%%
...
%
To evaluate the linkage disequilibrium (LD) of non-synonymous \emph{Vgsc} mutations with the two most widespread \emph{kdr} resistance mutations (\texttt{L995S/F}), the D1 statistic was calculated using haplotypes @@how?.
%%
